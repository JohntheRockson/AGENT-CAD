...
//! On first call to [`OcctKernel::new()`] (or [`OcctKernel::precompile`]), the
//! WASM is decompressed and compiled by wasmtime. The compiled module is shared
//! process-wide and reused from wasmtime's on-disk cache on later process starts.
//! Shape data lives in an arena inside the WASM sandbox, referenced by opaque
//! [`ShapeHandle`] values.
