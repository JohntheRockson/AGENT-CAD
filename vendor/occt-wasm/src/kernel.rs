...
use wasmtime::{Cache, CacheConfig, Engine, Instance, Linker, Memory, Module, Store, TypedFunc};
...
/// This is decompressed once during the first [`OcctKernel::precompile`] /
/// [`OcctKernel::new`] in the process.
static WASM_BINARY: &[u8] = include_bytes!("occt-wasm.wasm.br");

/// Process-wide compiled OCCT module. `Engine` and `Module` are cheap to clone
/// (`Arc` internally); compiling the 21 MB WASM is not.
struct CompiledWasm {
    engine: Engine,
    module: Module,
}

static COMPILED: OnceLock<CompiledWasm> = OnceLock::new();
static COMPILE_LOCK: Mutex<()> = Mutex::new(());

fn compile_occt_wasm() -> OcctResult<CompiledWasm> {
...
fn compiled_wasm() -> OcctResult<&'static CompiledWasm> {
    ...
    let compiled = compile_occt_wasm()?;
    let _ = COMPILED.set(compiled);
    COMPILED
        .get()
        .ok_or_else(|| OcctError::Memory("OCCT WASM module was not stored".to_owned()))
}
...
impl OcctKernel {
    /// Compile (or load from disk cache) the embedded OCCT WASM module.
    ///
    /// Safe to call from multiple threads. Compilation happens once per process;
    /// later calls are a no-op. Call this at process start so the first
    /// [`OcctKernel::new`] does not stall a request.
    pub fn precompile() -> OcctResult<()> {
        compiled_wasm().map(|_| ())
    }

    /// Create a new OCCT kernel instance.
    ///
    /// Instantiates the process-wide compiled module (see [`Self::precompile`])
    /// and initializes the OCCT runtime. Instantiation is cheap compared to
    /// compiling the ~21 MB WASM (~100–500 ms in release after a warm disk cache;
    /// minutes if Cranelift itself was built unoptimized).
    #[allow(clippy::too_many_lines)]
    pub fn new() -> OcctResult<Self> {
        let compiled = compiled_wasm()?;
        let mut store = Store::new(&compiled.engine, ());
        let mut linker = Linker::new(&compiled.engine);
        // Stub out any Emscripten runtime imports the WASM binary references
        // but that are unused by pure CAD operations (e.g. image/canvas APIs).
        linker.define_unknown_imports_as_traps(&compiled.module)?;
